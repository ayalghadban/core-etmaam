                            ...
                            @foreach ($jobs as $key => $job)
                                <div class="col-md-12">
                                    <div class="single-job @if ($loop->last) mb-0 @endif">
                                        ...
                                        <h3><a href="{{ route('front.careerdetails', [$job->slug]) }}"
                                                class="title">{{ convertUtf8($job->title) }}</a></h3>
                                        ...
                                        <p class="deadline"><strong><i class="far fa-calendar-alt"></i>
                                                {{ __('Deadline') }}:</strong> {{ $deadline }}</p>
                                        <p class="education"><strong><i class="fas fa-graduation-cap"></i>
                                                {{ __('Educational Experience') }}:</strong> {!! strlen(strip_tags($job->educational_requirements)) > 110
                                                    ? mb_substr(strip_tags($job->educational_requirements), 0, 110, 'utf-8') . '...'
                                                    : strip_tags($job->educational_requirements) !!}</p>
                                        <p class="experience"><strong><i class="fas fa-briefcase"></i>
                                                {{ __('Work Experience') }}:</strong> {{ $job->experience }}</p>
                                    </div>
                                </div>
                            @endforeach
                        @endif
                        ...
                        <div class="searchbar-form-section">
                            <form action="{{ route('front.career') }}">
                                <div class="searchbar">
                                    ...
                                    <input name="term" type="text" placeholder="{{ __('Search Jobs') }}"
                        ...
                        <div class="category-lists job">
                            ...
                            <ul>
                                <li class="single-category {{ empty(request()->input('category')) ? 'active' : '' }}">
                                    <a href="{{ route('front.career') }}">{{ __('All') }}
                                        <span>({{ $jobscount }})</span></a>
                                </li>
                                @foreach ($jcats as $key => $jcat)
                                    <li
                                        class="single-category {{ $jcat->id == request()->input('category') ? 'active' : '' }}">
                                        <a
                                            href="{{ route('front.career', ['category' => $jcat->id, 'term' => request()->input('term')]) }}">{{ convertUtf8($jcat->name) }}
                                            <span>({{ $jcat->jobs()->count() }})</span></a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
