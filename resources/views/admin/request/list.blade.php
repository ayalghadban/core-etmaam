...
<!--              data-target="#createModal"><i class="fas fa-plus"></i> Add Request</a>-->
<!--{{--            <button class="btn btn-danger float-right btn-sm mr-2 d-none bulk-delete"--}}-->
<!--{{--              data-href="{{route('admin.request.bulk.delete')}}"><i class="flaticon-interface-5"></i> Delete</button>--}}-->
<!--          </div>-->
        </div>
      </div>
          ...
          <div class="col-lg-12">
            <div class="table-responsive">
              <table class="table table-striped mt-3" id="basic-datatables">
                ...
                <tbody>
                  @foreach ($service_requests as $package)
                  <tr>
                    ...
                    <td>
                      {{strlen(convertUtf8($package->company_name)) > 90 ? convertUtf8(substr($package->company_name, 0, 90)) . '...' : convertUtf8($package->company_name)}} - ({{$package->company_type}})
                      <br>
                       {{$package->emp_name}} - {{$package->company_city}} 
                    </td>
                    
                     <td>
                      {{$package->emp_mobile}} <br>
                    {{$package->emp_email}} 
                    <br>
                                               {{$package->suitable_time}}
                    </td>


                    <td>
                      <button class="btn btn-secondary btn-sm" data-toggle="modal"
                        data-target="#detailsModal{{$package->id}}"><i class="fas fa-eye"></i> عرض التفاصيل</button>
                    </td>
                    ...
                    <td>
                      <form id="featureForm{{$package->id}}" class="d-inline-block"
                        action="{{route('admin.request.changed')}}" method="post">
                        @csrf
                        <input type="hidden" name="request_id" value="{{$package->id}}">
                        <select class="form-control {{$package->active == 1 ? 'bg-success' : 'bg-danger'}}"
                          name="active" onchange="document.getElementById('featureForm{{$package->id}}').submit();">
                          <option value="1" {{$package->active == 1 ? 'selected' : ''}}>تمت الاضافة</option>
                          <option value="0" {{$package->active == 0 ? 'selected' : ''}}>جديد</option>
                        </select>
                      </form>
                    </td>
                    ...
                    <!--  <form class="deleteform d-inline-block" action="{{route('admin.request.delete')}}" method="post">-->
                    <!--    @csrf-->
                    <!--    <input type="hidden" name="request_id" value="{{$package->id}}">-->
                  ...
                  <div class="modal fade" id="detailsModal{{$package->id}}" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                      <div class="modal-content">
                        ...
                        <div class="modal-body" id="MyDescription">
                            <p style="color:#fff;">
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="ajaxForm" class="modal-form" action="{{route('admin.request.store')}}" method="POST">
          @csrf
          ...
          <div class="form-group">
            ...
            <select name="maincategory_id" id="package_category_id" class="form-control" disabled>
            ...
            <div class="form-group">
                ...
                <select name="category_id" id="sub_category_id" class="form-control" disabled>
          ...
          <div class="form-group">
            ...
            <div class="selectgroup w-100">
              <label class="selectgroup-item">
                <input type="radio" name="active" value="true" class="selectgroup-input" checked>
                <span class="selectgroup-button">Yes</span>
              </label>
              <label class="selectgroup-item">
                <input type="radio" name="active" value="false" class="selectgroup-input">
...
<script>
  $(document).ready(function() {
    $("select[name='language_id']").on('change', function() {
      ...
      let langId = $(this).val();
      let url = "{{url('/')}}/admin/request/" + langId + "/get_categories";
      ...
      $("select[name='maincategory_id']").on('change', function() {
          ...
          let langId = $(this).val();
          let url = "{{url('/')}}/admin/request/" + langId + "/get_subcategories";

          $.get(url, function(data) {
              let options = `<option value="" disabled selected>Select a sub-category</option>`;

              if (data.length == 0) {
                  options += `<option value="" disabled>${'No SubCategory Exists'}</option>`;
              } else {
                  for (let i = 0; i < data.length; i++) {
                      options +=`<option value="${data[i].id}">${data[i].name}</option>`;
                  }
